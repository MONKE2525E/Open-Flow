...
| --- | --- | --- |
| Shared actions | `btn-primary`, `btn-ghost`, `btn-danger`, `btn-compact`, `add-btn`, `btn-clear`, `load-older-btn`, `notification-test-send-button`, `simulation-provider-button` | The first four are global in `src/ui.css`. The remaining tokens are layout or feature modifiers on an action. |
...
| Setup | `btn-skip`, `btn-open`, `btn-got-key`, `show-btn`, `help-btn`, `tryit-reset`, `lang-clear` | Setup flow has deliberate pacing and spatial constraints. Do not swap in a primary button without checking the flow. |

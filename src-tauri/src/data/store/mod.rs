...
fn merged_contextual_formatting_value(
    contextual_caps: Option<bool>,
    auto_spacing: Option<bool>,
) -> bool {
    contextual_caps.unwrap_or(true) && auto_spacing.unwrap_or(true)
}

/// Migrates the two legacy switches into the single atomic formatter switch.
/// AND is intentional: a user who disabled either old mutation has opted out
/// of the combined behavior. The legacy keys are mirrored for one release so
/// downgrading the app does not silently turn either behavior back on.
pub fn migrate_contextual_formatting(settings: &SettingsHandle) -> Result<(), String> {
    let enabled = settings
        .get(CONTEXTUAL_FORMATTING)
        .and_then(|value| value.as_bool())
        .unwrap_or_else(|| {
            merged_contextual_formatting_value(
                settings
                    .get(CONTEXTUAL_CAPS)
                    .and_then(|value| value.as_bool()),
                settings.get(AUTO_SPACING).and_then(|value| value.as_bool()),
            )
        });
    let value = serde_json::json!(enabled);
    let already_migrated = settings.get(CONTEXTUAL_FORMATTING).as_ref() == Some(&value)
        && settings.get(CONTEXTUAL_CAPS).as_ref() == Some(&value)
        && settings.get(AUTO_SPACING).as_ref() == Some(&value);
    if already_migrated {
        return Ok(());
    }
    settings.set(CONTEXTUAL_FORMATTING, value.clone())?;
    settings.set(CONTEXTUAL_CAPS, value.clone())?;
    settings.set(AUTO_SPACING, value)?;
    settings.save()
}

...
#[test]
fn contextual_formatting_legacy_matrix_is_conservative() {
    assert!(merged_contextual_formatting_value(Some(true), Some(true)));
    assert!(!merged_contextual_formatting_value(Some(true), Some(false)));
    assert!(!merged_contextual_formatting_value(Some(false), Some(true)));
    assert!(!merged_contextual_formatting_value(
        Some(false),
        Some(false)
    ));
    assert!(merged_contextual_formatting_value(None, None));
}
...
fn setting_audit_empty_store_resolves_to_documented_defaults() {
    ...
    assert!(
        ...
        "contextual formatting defaults on"
    );

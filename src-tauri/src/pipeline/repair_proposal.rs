                    ...
                    DictionaryOperation::Update => {
                        ...
                        format!(
                            ...
                            truncate_for_display(
                                mistake
                                    .as_deref()
                                    .or(current.mistake.as_deref())
                                    .unwrap_or(&current.term)
                            ),
...
pub(super) fn setting_label(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        store::CONTEXTUAL_FORMATTING => "Smart spacing & capitalization",
    ...
    match action {
        ...
        RepairAction::Dictionary {
            ...
            DictionaryOperation::Update => {
                term.as_deref()
                    .is_some_and(|v| !v.trim().is_empty() && evidence.contains(&v.to_lowercase()))
                    && mistake.as_deref().is_none_or(|v| {
                        !v.trim().is_empty() && evidence.contains(&v.to_lowercase())
                    })
            }
            DictionaryOperation::Remove => dictionary_id
                .and_then(|id| snapshot.dictionary.iter().find(|entry| entry.id == id))
                .is_some_and(|entry| {
                    evidence.contains(&entry.term.to_lowercase())
                        || entry
                            .mistake
                            .as_deref()
                            .is_some_and(|value| evidence.contains(&value.to_lowercase()))
                }),
        },
...
pub(super) fn validate_action(
    ...
    match &action {
        ...
        RepairAction::Setting {
            ...
            if matches!(
                key.as_str(),
                store::CLEANUP_ENABLED | store::CONTEXTUAL_FORMATTING | store::CAPS_LOCK_UPPERCASE
            ) && !value.is_boolean()
            {

...
pub(super) fn replace_phrase_with_marker(
    ...
    for start in 0..=lower.len().saturating_sub(needle.len()) {
        ...
        // Unicode lowercasing can expand one source character into several
        // folded characters. Only accept matches that begin and end on source
...
mod tests {
    ...
    fn private_text_replaces_each_marker_without_clipboard_content() {
        ...
        assert_eq!(private.matches("secret clipboard contents").count(), 0);
        assert_eq!(
            private

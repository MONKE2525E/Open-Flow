...
pub(super) async fn transcribe_any(
    ...
    if provider_id == store::LOCAL {
        let manager = app
            .try_state::<crate::local_stt::LocalTranscriptionManager>()
            .ok_or_else(|| anyhow::anyhow!("Local transcription manager is unavailable"))?
            .inner()
            .clone();
...
pub(super) async fn run_transcription(
    ...
    let corroborated_candidate = alternate_result.as_ref().is_some_and(|(alternate, _, _)| {
        let primary = prepare_transcript_text(&raw, false, true);
        let alternate = prepare_transcript_text(alternate, false, true);

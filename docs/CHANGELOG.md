...
- **Voice detection is now automatic and adapts per microphone.** Verenu learns how each input device sounds from real dictation outcomes and adjusts its own sensitivity, starting from a whisper-friendly default. Switching microphones switches profiles; a new device needs no setup.
- Detection is no longer pass/fail. A recording is classified as speech, **borderline**, or silence, and borderline audio — quiet speech and whispers — is transcribed rather than discarded. Level is judged against each recording's own noise floor instead of a fixed loudness threshold, so a whisper on a quiet mic is no longer treated the same as a whisper on a noisy one.
- **Removed the microphone gain slider** along with microphone calibration. Capture gain is now a fixed 3.5x pre-amp — the value that has been the default since 0.10.0 — so recordings sound the same for anyone who never moved the slider. Nothing in the audio path scales detection thresholds any more except the learned per-device sensitivity.
- **Removed microphone calibration** (setup step and Audio settings panel). It is replaced by Settings → Audio → **Voice detection: Automatic**, with a single **Reset learned sensitivity** action that clears every device profile and leaves microphone gain and device selection untouched. The first-run wizard is one step shorter.
- "Audio too quiet" and the 0.7s minimum are no longer rejections. Only genuinely unusable captures are discarded outright — no audio, a corrupt device, or a key tap under 0.15s — so short dictations like "yes" or "thanks" go through.
- **Retry on the dictation pill now reuses the original audio** for every speech-related rejection instead of asking you to repeat yourself, and bypasses voice detection. A retry that then produces real speech is treated as strong evidence the detector was too strict for that microphone.
- Fixed the pill's Retry button appearing to do nothing: every failure now explains itself on the pill instead of silently dismissing it.
- Fixed a retry being able to replay an *older* recording when a newer one failed early. A new recording now always supersedes the previous retryable audio.
